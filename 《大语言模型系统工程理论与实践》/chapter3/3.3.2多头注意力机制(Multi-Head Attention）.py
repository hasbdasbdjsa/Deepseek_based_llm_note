...
v = torch.randn(batch_size, len_k, d_model)

# 创建掩码（可选）
mask = torch.zeros(batch_size, len_q, len_k).bool()  # False表示不掩码

# 初始化多头注意力机制
multihead_attn = MultiHeadAttention(d_model=d_model, n_heads=n_heads)

# 前向传播
output, attn = multihead_attn(q, k, v, mask)

print(f"输出形状: {output.shape}")  # [batch_size, len_q, d_model]
print(f"注意力权重形状: {attn.shape}")  # [batch_size, n_heads, len_q, len_k]
